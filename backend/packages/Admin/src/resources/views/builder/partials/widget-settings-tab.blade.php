{{-- Shared settings tab content for all widgets --}}
{{-- Include this in the settings tab of each widget --}}
...
<div>
    ...
    <div class="space-y-2 px-2">
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                x-model="currentWidget().visibility.desktop"
                @change="sync()"
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                x-model="currentWidget().visibility.tablet"
                @change="sync()"
                class="rounded border-neutral-300 text-neutral-900 focus:ring-neutral-900 focus:ring-offset-0">
            <span class="flex items-center gap-1.5 text-xs text-neutral-700">
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                x-model="currentWidget().visibility.mobile"
                @change="sync()"
                class="rounded border-neutral-300 text-neutral-900 focus:ring-neutral-900 focus:ring-offset-0">
            <span class="flex items-center gap-1.5 text-xs text-neutral-700">
...
<div class="bg-blue-50 border border-blue-200 rounded-lg p-2.5">
    ...
    <p class="text-xxs text-blue-700">Décochez un appareil pour masquer le widget sur cette taille d'écran.</p>
</div>
...
<div class="border border-neutral-200 rounded-lg p-3 mt-3 space-y-0">
    ...
    @include('admin::builder.partials.spacing-control', [
        ...
        'type' => 'padding',
        'model' => 'currentWidget().props.padding'
    ])

    @include('admin::builder.partials.spacing-control', [
        'label' => 'Margin extérieur',
        'type' => 'margin',
        'model' => 'currentWidget().props.margin'
    ])
</div>
