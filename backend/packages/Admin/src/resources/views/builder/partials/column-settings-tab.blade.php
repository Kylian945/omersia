{{-- Shared settings tab content for columns --}}
{{-- Include this in the settings tab of columns --}}
...
<div>
    ...
    <div class="space-y-2 px-2">
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                x-model="currentColumn().visibility.desktop"
                @change="sync()"
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                x-model="currentColumn().visibility.tablet"
                @change="sync()"
                class="rounded border-neutral-300 text-neutral-900 focus:ring-neutral-900 focus:ring-offset-0">
            <span class="flex items-center gap-1.5 text-xs text-neutral-700">
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox"
                x-model="currentColumn().visibility.mobile"
                @change="sync()"
                class="rounded border-neutral-300 text-neutral-900 focus:ring-neutral-900 focus:ring-offset-0">
            <span class="flex items-center gap-1.5 text-xs text-neutral-700">
...
<div class="bg-blue-50 border border-blue-200 rounded-lg p-2.5">
    ...
    <p class="text-xxs text-blue-700">Décochez un appareil pour masquer la colonne sur cette taille d'écran.</p>
</div>
...
<div class="border border-neutral-200 rounded-lg p-3 mt-3 space-y-0">
    ...
    @include('admin::builder.partials.spacing-control', [
        ...
        'type' => 'padding',
        'model' => 'currentColumn().padding'
    ])

    @include('admin::builder.partials.spacing-control', [
        'label' => 'Margin extérieur',
        'type' => 'margin',
        'model' => 'currentColumn().margin'
    ])
</div>
