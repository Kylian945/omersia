...
@section('title', 'Modifier le taux de taxe')
@section('page-title', 'Modifier le taux de taxe')
...
<div class="space-y-4">
    ...
    <div class="rounded-2xl bg-white border border-black/5 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.settings.taxes.rates.update', [$taxZone, $taxRate]) }}">
            @csrf
            @method('PUT')

            @include('admin::settings.taxes._rate-form')
