...
@section('content')
    @php
        ...
        $initialName = old('name', $t?->name ?? '');
        $initialIsActive = (bool) old('is_active', $product->is_active);
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" x-data="productCreateForm({
        ...
        data-product-id="{{ $product->id }}" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        <div class="truncate text-base font-semibold text-gray-900"
                            x-text="productName !== '' ? productName : 'Produit #{{ $product->id }}'"></div>
