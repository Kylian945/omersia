...
<template x-if="currentWidget().type === 'accordion'">
    <div x-data="{ activeWidgetTab: 'content', openAccordionItems: {} }"
        class="space-y-3 mt-0"
        x-init="initAccordionProps()"
        :key="'accordion-widget-' + selected.widgetId">
        @include('admin::builder.partials.widget-tabs-nav')

        {{-- Content tab --}}
        <div x-show="activeWidgetTab === 'content'" class="space-y-3">
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-neutral-700">Éléments</span>
            ...
            <template x-for="(item, index) in currentWidget().props.items" :key="index">
                <div class="border border-neutral-200 rounded-lg bg-neutral-50">
                    <div class="flex items-center justify-between px-3 py-2">
                        <button type="button"
                            @click="openAccordionItems[index] = !openAccordionItems[index]"
                            ...
                            <div class="min-w-0">
                                <div class="text-xs font-medium text-neutral-800 truncate">
                                    <span x-text="currentWidget().props.items[index].title || `Élément #${index + 1}`"></span>
                                </div>
                        ...
                        <div class="ml-2 flex items-center gap-1 shrink-0">
                            <button type="button"
                                ...
                                <x-lucide-chevron-down
                                    ...
                                    x-bind:class="openAccordionItems[index] ? 'rotate-180' : ''"
                                />
                            </button>

                            <button type="button" @click="removeAccordionItem(index)"
                    ...
                    <template x-if="openAccordionItems[index]">
                        <div class="px-3 pb-3 space-y-3">
                            ...
                            }" x-init="initQuillForAccordionItem()">
                                <label class="block">
            ...
            <div x-show="!currentWidget().props.items || currentWidget().props.items.length === 0"
                ...
                Aucun élément. Cliquez sur "+ Ajouter" pour créer un élément.
            </div>
        </div>
