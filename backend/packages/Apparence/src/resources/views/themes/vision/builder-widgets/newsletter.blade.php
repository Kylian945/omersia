{{-- Widget: Newsletter --}}
<template x-if="currentWidget().type === 'newsletter'" x-init="initNewsletterProps(); sync()">
    <div x-data="{
            ...
            initDescriptionQuill() {
                this.$nextTick(() => {
                    if (!this.$refs.newsletterDescriptionEditor || this.quillInstance) return;

                    const Quill = window.Quill;
                    if (!Quill) return;

                    this.quillInstance = new Quill(this.$refs.newsletterDescriptionEditor, {
                        theme: 'snow',
                        modules: {
                            toolbar: [
                                [{ 'header': [2, 3, 4, false] }],
        ...
        :key="'newsletter-widget-' + selected.widgetId"
        ...
        <div x-show="activeWidgetTab === 'content'" class="space-y-3">
            ...
            <div class="block">
                ...
                <div x-ref="newsletterDescriptionEditor" class="bg-white"></div>
            </div>
            <label class="block">
