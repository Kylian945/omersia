...
export function AccountAddresses({ initialAddresses }: Props) {
  ...
  const handleSetDefaultShipping = async (id: number) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/account/addresses/${id}/default-shipping`, {
        method: "POST",
        credentials: "include",
        headers: {
          Accept: "application/json",
      ...
      if (!res.ok) {
        alert("Impossible de définir cette adresse comme livraison par défaut.");
        return;
      }
  ...
  const handleSetDefaultBilling = async (id: number) => {
    try {
      setLoading(true);
      const res = await fetch(`/api/account/addresses/${id}/default-billing`, {
        method: "POST",
        credentials: "include",
        headers: {
          Accept: "application/json",
      ...
      if (!res.ok) {
        alert(
          "Impossible de définir cette adresse comme facturation par défaut."
        );
        return;
      }
